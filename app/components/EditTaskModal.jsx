...
export const EditTaskModal = ({handleCloseEditModal, text, handleSaveEdit}) => {
    ...
    return (
      <div className="modal-overlay h-[100vh] w-[100%] bg-black bg-opacity-70 absolute left-0 top-0 flex flex-col items-center" onClick={(e) => handleCloseEditModal(e)}>
          ...
          <form className="flex flex-col w-[50%] rounded bg-white text-black p-5 mt-28" onSubmit={(e) => handleSaveEdit(e, newText)}>
              <input 
                ...
                autoFocus
              />
              <button className="h-8 bg-green-500 rounded text-gray-50 hover:bg-green-600">Save Edit</button>
          </form>
      </div>
    )
  }